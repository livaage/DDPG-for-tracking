...
md = pd.read_csv('/home/lhv14/new_md_hitbased.csv',  header=[0,1], index_col=[0, 1,2])
md = md.reset_index()
...
class TrackMLEnv():
    ...
    def __init__(self,
        ...
        self._observation_space = akro.Box(low=np.array([-300, 0, -300, 0, -300, -300, -300, 0, 0, 0, 0]), high=np.array([300, 120, 300, 120, 300, 300, 300, 120, 120, 120, 8]), dtype=np.float64)
        self._action_space = akro.Box(low=np.array([-100, 0]),
    ...
    def reset(self):
        ...
        # only tracks with more than six hits 
        ...
        # uncomment this if considering only the inner part of the detector (excludes some missing hits )
        ...
        #uncomment when more than one file used 
        #print(self.event)
        ...
        #if np.abs(dz) > 15: 
        #    self.reset()
        # module_id = next_hit.discrete_module_id 
        self._point = next_hit[['z', 'r']].values 
        #dr = 0 
        #dz = 0
        self.dr_buffer = [dr]
        self.dz_buffer = [dz]
        # #self.phi = calc_phi(dr,dz)

        self.intersect_points = self.find_intersects(start_hit, next_hit)
     

        next_intersect = self.intersect_points[1]
        next_three_hits = self.event.iloc[(np.sqrt((self.event.r - next_intersect[1])**2 + (self.event.z - next_intersect[0])**2).values).argsort()[:3], ]        # #self.intersect = self.initial_intersects
       ...
       # observation = np.append(observation, next_three_hits.z.values)
       # observation = np.append(observation, next_three_hits.r.values)
        observation = np.append(observation, self.num_track_hits)
    ...
    def step(self, action):
        ...
        #
        try: 
            naive_estimate =  self.intersect_points[self.num_track_hits]
            
            #print("na", naive_estimate)
        except: 
            naive_estimate = [self._point[0]+ np.max(self.dz_buffer[-1], 0), self._point[1] + np.max(self.dr_buffer[-1], 0)]
        



        #if len(np.shape(self.intersect)) > 1: 
        #    try: 
        #        naive_estimate = self.intersect[0] 
        #    except: 
        #        naive_estimate = [self._point[0] + self.dz_buffer[-1], self._point[1] + self.dr_buffer[-1]]
        #else: 
        #    naive_estimate = self.intersect
        ...
        predicted_point_z = np.clip(a_clipped[0] + naive_estimate[0], -300, 300)
        predicted_point_r = np.clip(a_clipped[1]+ naive_estimate[1],  0, 120)

        next_intersect = self.intersect_points[self.num_track_hits]

        next_three_hits = self.event.iloc[(np.sqrt((self.event.r - next_intersect[1])**2 + (self.event.z - next_intersect[0])**2).values).argsort()[:3], ]        # #self.intersect = self.initial_intersects


        #predicted_point_z = np.clip(self._point[0] + a_clipped[0] + np.mean(self.dz_buffer),  -300, 300)
        #predicted_point_r = np.clip(self._point[1] + a_clipped[1] + np.mean(self.dr_buffer), 0, 120)
        ...
        #self.intersect = self.find_intersects(self.previous_state, self.state)
        #if len(self.intersect) ==0: 
        #    try: 
        #        self.intersect = self.initial_intersects[self.num_track_hits -2]
        #    except: 
        #        self.intersect = []
